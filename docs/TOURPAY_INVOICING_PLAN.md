...
- **Known limits closed (2026-09-25):**
  ...
  - **Database protection:** on a server with binary logging on, MySQL only lets the app create the append-only triggers if `log_bin_trust_function_creators = 1`. `php artisan money:protect` installs them (safe to run again); `/health` turns red in production until they exist.
