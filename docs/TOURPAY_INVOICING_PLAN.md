...
- **Known limits closed (2026-09-25):**
  - **Cross-currency invoices:** an invoice keeps its own currency and is never converted. When its booking is in another currency, the booking's paid amount is credited at that day's rate: the business's own rates first, otherwise the free daily feed at open.er-api.com (kept per day in `bc_fx_rates`, refreshed nightly by `fx:refresh`; attribution "Rates By Exchange Rate API" is shown on the statement). With no rate known nothing is guessed: the payment is not linked and the nightly check lists it. A booking payment in one currency is allocated to its invoice in the other the same way.
  - **Commission:** money a business collects itself (its own gateway, bank, cash) accrues the platform's commission share, in the currency it was paid in (currencies are never mixed). Fully paid owes exactly the booking's commission; a refund gives its share back. It shows on the statement as "Commission owed to platform" (an account, not cash). In the payout currency it is held back from the next payout and settled out of what the platform still holds; a debt in another currency is never set against money in this one. `commission:settle {vendor} {currency} {amount}` records commission paid outside the platform.  Money already in the ledger before commission existed gets its commission once, from the backfill; the nightly check flags any payment that missed it.
  - **One payment schedule:** when a booking has a live invoice in the same currency, its plan and the invoice's instalments are the same schedule. An edit on either screen (build, clear, waive, split, deposit) is copied to the other, paid rows follow the ledger earliest first, and a refund takes the last ones back to pending.
  - **Nightly check at scale:** every check is one set-based SQL query (no query per payment); it stays a handful of queries for thousands of payments (tested) and adds two checks: payments that could not be linked for want of a rate, and, in production, that the database triggers exist.
  - **Tamper-evident on any server:** every ledger row carries a hash of its own content and of the row before it (one chain per business), and an anchor remembers each chain's last row. A changed amount, a deleted row, a row slipped in, or newest rows removed all break the chain, and the nightly check turns `/health` red. This needs nothing from the server. Database triggers that also refuse the change outright are an optional extra layer: on a server with binary logging they need `log_bin_trust_function_creators = 1`, then `php artisan money:protect`; `/health` reports which layers are active.
